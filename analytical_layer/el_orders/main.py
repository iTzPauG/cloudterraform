...
def sync_table(db_conn_manager, table_name):
    ...
    if schema:
        ...
        try:
            ...
            logging.info(
                f"Successfully wrote {len(data)} records to BigQuery table {table_name}."
            )
        except Exception as e:
            logging.error(
                f"Error during BigQuery synchronization for {table_name}: {e}"
            )
    else:
...
if __name__ == "__main__":
    ...
    OLAP_DB_CONFIG = {"project": os.getenv("GCP_PROJECT")}
