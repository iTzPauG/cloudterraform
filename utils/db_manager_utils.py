...
class DbManager:
    ...
    def get_operational_data(self, table):
        try:
            ...
            processed_data = [
                tuple(self.convert_value(value) for value in row) for row in data
            ]
    ...
    def write_tables_to_analytical_db(
        self, data, table, dataset_id, schema, write_disposition="WRITE_TRUNCATE"
    ):
        try:
            ...
            rows_to_insert = [
                dict(zip([field.name for field in schema], row)) for row in data
            ]

            job = client_olap.load_table_from_json(
