...
class EventsManager:
    ...
    def consume_messages(self):
        ...
        if not self.subscriber:
            logging.error(
                "Subscriber is not initialized. Call create_subscriber() first."
            )
            return
        try:
            request = pubsub_v1.types.PullRequest(
                subscription=self.subscriber_path, max_messages=150
            )
            ...
            for received_message in response.received_messages:
                ...
                logging.info(
                    f"Consumed message: {received_message.message.data.decode('utf-8')}"
                )
